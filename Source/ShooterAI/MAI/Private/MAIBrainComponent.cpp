   ...
   // UE_LOG(LogTemp, Warning, TEXT("Brain Ticking! AI owner is %s"), *AIOwner->GetName());
    ...
    if(/*bShouldExplore*/ AIOwner->GetPawn() && bGotReward)
    {
       ...
       // AStaticMeshActor* closest = FindClosestBox(((AMAIController*) AIOwner)->GetGameRep()->GetSeenWhiteBoxes());
        ...
        if((APlayerController*)AIOwner->GetPawn()->GetController())
...
bool UMAIBrainComponent::GetGotReward()
{
    return bGotReward;
}

void UMAIBrainComponent::SetGotReward( bool bGReward )
{
    bGotReward = bGReward;
}

bool UMAIBrainComponent::GetLookForBox()
{
    return bLookForBox;
}

void UMAIBrainComponent::SetLookForBox(bool bLForBox)
{
    bLookForBox = bLForBox;
}

bool UMAIBrainComponent::GetShouldFire()
{
    return bShouldFire;
}

void UMAIBrainComponent::SetShouldFire(bool SFire)
{
    bShouldFire = SFire;
}

float UMAIBrainComponent::GetScoreReward()
{
    return ScoreReward;
}

void UMAIBrainComponent::SetScoreReward(float Score)
{
    ScoreReward = Score;
}
